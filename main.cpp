...
public:
    void addReport(int reserve_id, int usr_id, int car_id, float total_charge,
        ...
        while (temp != nullptr) {
            if (temp->reservationId == reserve_id) {
                // Update existing report
                temp->userId = usr_id;
                temp->carId = car_id;
                temp->total_amount_paid = total_charge;
                temp->extra_charge = penalty;
                temp->status = status_of_rsv;
                temp->end_date = end_date;
                temp->reported_by = reporter;
                cout << "Reservation with ID " << reserve_id << " updated successfully.\n";
                return;
            }
            temp = temp->next;
        }

        // Add new report if ID doesn't exist
        Node* newNode = new Node(reserve_id, usr_id, car_id, total_charge,
                                 penalty, status_of_rsv, end_date, reporter);
        newNode->next = head;
        head = newNode;
        cout << "New reservation with ID " << reserve_id << " added successfully.\n";
    }

    void addDummyData() {
        addReport(1, 101, 1, 100.0, 0.0, "Completed", "2024-12-26");
        addReport(2, 102, 2, 150.0, 50.0, "Cancelled", "2024-12-25", "Admin");
        addReport(3, 103, 3, 80.0, 0.0, "Pending", "2024-12-27");
        addReport(1, 101, 1, 120.0, 0.0, "Completed", "2024-12-26"); // Update existing report 1
    }
    ...
    void displayReports() {
        ...
        while (true) {
            ...
            cout << "\n===================== Reports =====================\n";
            cout << "1. All Reports\n";
            cout << "2. New Reservations\n";
            cout << "3. Completed Reservations\n";
            cout << "4. Cancelled Reservations\n";
            cout << "0. Back\n";
            ...
            cout << left << setw(12) << "Res. ID" << setw(10) << "User ID"
                 << setw(10) << "Car ID" << setw(15) << "Total Charge"
            ...
            while (temp != nullptr) {
                if (status != "All" && temp->status != status) {
                    temp = temp->next;
                    continue;
                }
                    found = true;
            ...
            if (!found) {
                cout << "No reports found for the selected status: " << status << endl;
            }
    ...
    /* 
        ...
        It helps to identify overdue reservations.
    */
...
void viewAllUsers(sqlite3 *DB, bool isManager = false) {
    ...
    while (continu == 'y' || continu == 'Y') {
        ...
        // Table Header
        cout << "\n=========================================" << endl;
        cout << left << setw(10) << "User ID" << setw(20) << "Name" << setw(30)
         << "Email" << setw(15) << "Phone" << setw(30) << "Address" << setw(15) << "Role" << endl;
        cout << string(110, '-') << endl;
...
void fireEmployee(sqlite3 *DB) {
    ...
    if (confirmation == 'y' || confirmation == 'Y') {
        ...
        cout << "Employee firing cancelled." << endl;
    }
}
...
void viewReservationDetails(sqlite3 *DB) {
    ...
    if (choice == 1) {
        ...
        string status;
        cout << "Enter Status (Pending, Completed, or Cancelled): ";
        cin.ignore();
        getline(cin, status);
    ...
    if (sqlite3_prepare_v2(DB, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        ...
        cout << left << setw(15) << "Res. ID" 
             << setw(10) << "User ID" 
             << setw(20) << "Car (Producer)" 
             << setw(15) << "Model"
             << setw(20) << "License Plate" 
             << setw(12) << "End Date"
             << setw(15) << "Total Price" 
             << setw(15) << "Extra Charge" 
             << setw(12) << "Status" << endl;
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            // Display the row in table format
            cout << left << setw(15) << reservation_id 
                 << setw(10) << user_id 
                 << setw(20) << producer 
                 << setw(15) << model 
                 << setw(20) << license_plate 
                 << setw(12) << end_date 
                 << setw(15) << total_price 
                 << setw(15) << extra_charge 
                 << setw(12) << status << endl;
            cout << string(135, '-') << endl;
        }
    ...
    while (true) {
        ...
        if (sqlite3_prepare_v2(DB, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                int user_id = sqlite3_column_int(stmt, 1);
                int car_id = sqlite3_column_int(stmt, 2);
                string end_date = (char *)sqlite3_column_text(stmt, 3);
                double total_price = sqlite3_column_double(stmt, 4);
                double extra_charge = sqlite3_column_double(stmt, 5);
                string status = (char *)sqlite3_column_text(stmt, 6);

                /* 
                    Skip displaying the content if option 2 (Overdue Reservations) is selected and
                    the end date is not overdue and also if the status is still pending
                 */
                if (choice == 2) {
                    if (status != "Pending")
                        continue;
                ...
                if (sqlite3_prepare_v2(DB, carQuery.c_str(), -1, &stmt_car, nullptr) == SQLITE_OK) {
                    if (sqlite3_step(stmt_car) == SQLITE_ROW) {
                        string producer = (char *)sqlite3_column_text(stmt_car, 1); // producer
                        string model = (char *)sqlite3_column_text(stmt_car, 2); // model
                        string license_plate = (char *)sqlite3_column_text(stmt_car, 3); // license_plate

                        // Display reservation details in table format
                        cout << left << setw(15) << reservation_id
                             << setw(10) << user_id
                             << setw(20) << producer
                             << setw(15) << model
...
void viewPayments(sqlite3 *DB) {
    int choice = -1;
    string query;

    do {
        cout << "==================== Payment Records ====================" << endl;
        cout << "1. View All Payments." << endl;
        cout << "2. Search by Payment ID." << endl;
        ...
        if (choice == 0) {
            ...
            cin >> payment_id;
            query += "WHERE payment_id = " + to_string(payment_id) + ";";
        } else if (choice == 3) {
            int reservation_id;
            cout << "Enter Reservation ID: ";
            cin >> reservation_id;
            query += "WHERE reservation_id = " + to_string(reservation_id) + ";";
        } else {
        ...
        cout << left << setw(15) << "Payment ID"
             << setw(20) << "Reservation ID"
             << setw(15) << "Amount"
             << setw(20) << "Payment Method"
             << setw(15) << "Status"
             << setw(25) << "Payment Date" << endl;
        cout << string(110, '-') << endl;
        ...
        while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            int reservation_id = sqlite3_column_int(stmt, 1);
            double amount = sqlite3_column_double(stmt, 2);
            string payment_method = (char *)sqlite3_column_text(stmt, 3);
            string status = (char *)sqlite3_column_text(stmt, 4);
            string payment_date = (char *)sqlite3_column_text(stmt, 5);

            // Display payment details
            cout << left << setw(15) << payment_id
                 << setw(20) << reservation_id
                 << setw(15) << amount
                 << setw(20) << payment_method
                 << setw(15) << status
                 << setw(25) << payment_date << endl;
        }
...
void agentMenu(sqlite3 *DB, int employee_id){
    ...
    while(true){
        ...
        cout << "7. View available Cars." << endl;
        cout << "8. Search Car." << endl;
        cout << "0. Log Out." << endl;
        cout << "=============================================" << endl;
        cout << "Enter your choice: ";
        cin >> choice;
    ...
    while (true) {
        ...
        switch (choice) {
            case 1:
                ...
                cout << "Enter the secret admin key: ";
                cin >> key;
...
void viewCars(sqlite3 *DB, bool isManager = false) {
    ...
    do {
        ...
        if (isManager) {
            ...
            cout << "2. Cars Under Maintenance." << endl;
        ...
        while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            string updated_at = (char *)sqlite3_column_text(stmt, 7);

            // Display car details in a table row
...
void searchCars(sqlite3 *DB) {
    ...
    if (choice == 0){
        ...
        query = "SELECT * FROM cars WHERE produced_by LIKE '%" + searchInput + "%';";

    } else if (choice == 2) {
        cout << "Enter the Model name: ";
        getline(cin, searchInput);
        query = "SELECT * FROM cars WHERE model LIKE '%" + searchInput + "%';";
    } else {
    ...
    if (!car_exist) {
        cout << "No cars found matching your search criteria." << endl;
    }
...
int main() {
    ...
    sqlite3* DB;
    int exit = sqlite3_open("System.db", &DB);
    if (exit) {
...
const char* insertDummyData = R"(
    ...
    -- Insert Reservations
    INSERT INTO reservations (user_id, car_id, end_date, total_price, extra_charge, status)
    VALUES
        (4, 1, '2024-12-05', 250.0, 0.0, 'Completed'),
        (4, 2, '2024-12-15', 275.0, 10.0, 'Pending');
